...
describe("operational matter urgency", () => {
  ...
  it("uses firm-specific thresholds including a zero-day critical window", () => {
    expect(deriveMatterRisk([1], { criticalDays: 0, warningDays: 3 })).toBe(
      "YELLOW",
    );
    expect(deriveMatterRisk([4], { criticalDays: 0, warningDays: 3 })).toBe(
      "GREEN",
    );
  });
  it.each([
    ...
    expect(() => deriveMatterRisk([], thresholds)).toThrow(
      "INVALID_RISK_THRESHOLDS",
    );
  });
